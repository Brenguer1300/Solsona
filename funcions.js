// ============================================================
// funcions.js — Guia de Punts d'Interès (plantilla per a qualsevol poble)
// ============================================================
...
/**
 * @constant {string} POBLE
 * Nom del poble. **Punt únic de canvi**: en modificar aquesta constant,
 * el nom s'actualitza automàticament a:
 *   · Títol de la pestanya del navegador (document.title)
 *   · Capçalera principal (#titol-poble)
 *   · Capçalera del menú lateral (.titol-menu)
 *   · Peu del menú lateral (#peu-menu)
 *   · Etiquetes ARIA i missatges de log
 * No cal tocar cap HTML ni cap fitxer de dades.
 */
const POBLE = 'Solsona';
 ...
 * Es deriva de POBLE perquè dos desplegaments diferents (dos pobles al
 * mateix domini, per exemple) no comparteixin preferències d'idioma.
 */
const CLAU_IDIOMA_LOCAL = `${POBLE.toLowerCase().replace(/\s+/g, '-')}-idioma`;
...
function desarALocalStorage(clau, valor) {
    try {
        ...
        console.warn(`[${POBLE}] No s'ha pogut desar "${clau}" a localStorage:`, error);
        return false;
    }
}
...
function llegirDeLocalStorage(clau, valorDefecte = '') {
    try {
        ...
        console.warn(`[${POBLE}] No s'ha pogut llegir "${clau}" de localStorage:`, error);
        return valorDefecte;
    }
}
...
function actualitzarTextosDinamics() {
    ...
    document.title = `${POBLE} — ${traduir(UI['titol-app'])}`;

    // --- Nom del poble a la capçalera principal ---
    // Coincideix semànticament amb l'id de l'element (#titol-poble).
    const elTitol = document.getElementById('titol-poble');
    if (elTitol) {
        elTitol.textContent = POBLE;
    }

    // --- Nom del poble a la capçalera del menú lateral ---
    // Hi pot haver diversos ".titol-menu" (un per HTML), per si de cas.
    document.querySelectorAll('.titol-menu').forEach(el => {
        el.textContent = POBLE;
    });

    // --- Peu del menú lateral: "<Poble> · <Títol app traduït>" ---
    const elPeuMenu = document.getElementById('peu-menu');
    if (elPeuMenu) {
        elPeuMenu.textContent = `${POBLE} · ${traduir(UI['titol-app'])}`;
    }
...
function canviarIdioma(nouIdioma) {
    if (!IDIOMES_DISPONIBLES.includes(nouIdioma)) {
        console.warn(`[${POBLE}] Idioma no vàlid: "${nouIdioma}"`);
        return;
    }
    ...
    try {
        ...
        console.warn(`[${POBLE}] No s'ha pogut actualitzar la URL:`, error);
    }
    ...
    for (const oient of oientsCanviIdioma) {
        try {
            ...
            console.error(`[${POBLE}] Error en oient de canvi d'idioma:`, error);
        }
    }
}
...
function inicialitzarPaginaPrincipal() {
    ...
    console.info(`[${POBLE}] Pàgina principal inicialitzada. Idioma: ${idiomaActual}`);
}
...
function carregarDadesZona(idZona) {
    if (typeof ZONES === 'undefined') {
        console.error(`[${POBLE}] ZONES no està definit. Comprova zones.js.`);
        return null;
    }
...
function obtenirPuntsDeZona(idZona) {
    if (typeof PUNTS_INTERES === 'undefined') {
        console.error(`[${POBLE}] PUNTS_INTERES no està definit. Comprova punts.js.`);
        return [];
    }
...
function renderitzarMapaZona(zona, punts) {
    ...
    img.addEventListener('error', () => {
        console.warn(`[${POBLE}] No s'ha pogut carregar el mapa de zona: ${zona.arxiuMapa}`);
        img.remove();
...
function inicialitzarPaginaZona() {
    ...
    if (!idZona) {
        console.error(`[${POBLE}] zona.html: manca el paràmetre ?zona= a la URL.`);
    ...
    if (!zona) {
        console.error(`[${POBLE}] zona.html: zona "${idZona}" no existeix a zones.js.`);
        mostrarErrorPagina(UI && UI['error-zona'] ? traduir(UI['error-zona']) : 'Zona no trobada.');
        return;
    }
    ...
    const refrescarCapçaleraZona = () => {
        ...
        document.title = `${traduir(zona.nom)} — ${POBLE}`;
    };
    ...
    console.info(`[${POBLE}] Zona "${idZona}" inicialitzada. PIs: ${puntsDeZonaActual.length}`);
}
...
function renderitzarFitxaPunt(punt) {
    ...
    // --- Títol del document ---
    document.title = `${traduir(punt.nom)} — ${POBLE}`;
}
...
function inicialitzarPaginaPuntInteres() {
    ...
    if (!idPunt) {
        console.error(`[${POBLE}] punt-interes.html: manca el paràmetre ?pi= a la URL.`);
    ...
    if (!punt) {
        console.error(`[${POBLE}] punt-interes.html: PI "${idPunt}" no existeix a punts.js.`);
        mostrarErrorPagina(UI && UI['error-punt'] ? traduir(UI['error-punt']) : 'Punt d\'interès no trobat.');
        return;
    }
    ...
    console.info(`[${POBLE}] PI "${idPunt}" carregat: ${punt.nom.ca}`);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    switch (pagina) {
        ...
        default:
            console.warn(`[${POBLE}] Pàgina no reconeguda: "${pagina}"`);
    }
});
...
window.addEventListener('pageshow', (event) => {
    ...
    for (const oient of oientsCanviIdioma) {
        try {
            ...
            console.error(`[${POBLE}] Error en oient de canvi d'idioma (pageshow):`, error);
        }
    }

    console.info(`[${POBLE}] Restaurat del bfcache. Idioma: ${idiomaActual}`);
});
